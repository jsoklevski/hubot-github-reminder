...
exports.getRoomForNotifications = function(robot) {
  var roomObject = {
    message: {
      room: internals.ROOM_TO_SEND_NOTIFICATIONS
    }
  };
  var room = robot.adapter.client.rtm.dataStore.getChannelOrGroupByName(roomObject);
  if (!room) {
    room = robot.adapter.client.rtm.dataStore.getChannelGroupOrDMById(roomObject);
  }
  return room;
};
