...
internals.findUsersAndFireEvents = function (robot, pr, status_result, reminder) {
  ...
  var user = utils.lookupUserWithGithub(robot, send_notification_to);
  if (!user) {
    robot.logger.error('No mapped user for github user ' + send_notification_to);
  }
  else {
    robot.logger.info('Sending notification to ' + user.name);
    robot.emit('GithubPullRequestAssigned', new PullRequestObject(pull_request_object), user);
  }
};
