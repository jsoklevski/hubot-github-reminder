...
internals.findUsersAndFireEvents = function (_this, pr, status_result, reminder) {
  ...
  if (status_result.allChecksPassed) {
    ...
    _this.save(pr, reminder.action, reminder.assignee, true);
  }
  ...
  if (!user) {
    ...
    _this.robot.logger.info('Sending notification to ' + user.name);
    _this.robot.emit('GithubPullRequestAssigned', new PullRequestObject(pull_request_object), user);
  }
};
PrStatusCheckWorker.prototype.save = function(pullRequest, action, assignee, author_notified) {
  var reminders = this.getStatusChecks();
  if (!author_notified) {
  ...
  var reminders_to_keep = _.reject(reminders, function(reminder){
    return reminder.pr === pullRequest.number
      && reminder.repoName === pullRequest.head.repo.name
...
PrStatusCheckWorker.prototype.clearStatusCheck = function(reminder) {
  ...
  var reminders_to_keep = _.reject(reminders, {
    ...
    authorNotified: reminder.authorNotified
  });
