...
internals.findUsersAndFireEvents = function (_this, pr, status_result, reminder) {
  ...
  var assignee = utils.lookupUserWithGithub(_this.robot, reminder.assignee);
  if (status_result.allChecksPassed) {
    pull_request_object.action = 'assigned';
    if (!assignee) {
      _this.robot.logger.error('No mapped user for github user ' + reminder.assignee);
    }
    else {
  ...
  if (!author) {
    ...
    _this.robot.emit('GithubPRAuthorNotification',
      new PullRequestObject(pull_request_object), author, status_result.allChecksPassed, assignee);
  }
...
PrStatusCheckWorker.prototype.clearStatusCheck = function(reminder_to_be_removed) {
  ...
  var reminders_to_keep = _.reject(reminders,  function(reminder){
    return reminder.pr === reminder_to_be_removed.pr
      ...
      && reminder.assignee === reminder_to_be_removed.assignee;
  });
