...
function WebhookHandler(robot) {
  ...
  this.robot.router.post('/hubot/github-events', function(req, res) {
    ...
    _this.robot.logger.info(req.headers['X-GitHub-Event']);
      ...
      event = req.body;
      _this.robot.logger.info(req.body);
...
internals.onPullRequest = function(robot, event, pr_satus_check) {
  ...
  octo.fromUrl(pr.Links.statuses.href).fetch().then(function(checks) {
    ...
    var pullRequestObject = utils.formatPullRequest(pr, status_result);
    githubDataService.updatePullRequestsCache(robot, pullRequestObject, pr.state);
  });
