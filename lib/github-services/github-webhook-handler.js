...
internals.onPullRequestReview = function(robot, event) {
  ...
  if (event.action === 'submitted' || event.action === 'dismissed') {
    if (review.state !== 'commented') {
      internals.processNotificationsForReview(robot, pr, author, pr_cache, review, 'review');
    }
  }
};
...
internals.onPullRequestReviewComment = function(robot, event) {
  ...
  if (event.action === 'created') {
    internals.processNotificationsForReview(robot, pr, author, pr_cache, comment, 'comment');
  }
};

internals.processNotificationsForReview = function(
  robot, pr, author, pr_cache, review_comment, event_type) {
  ...
  repo.pulls(pr.number).fetch().then(function(pull_request) {
    var pull_request_object;
    if (event_type === 'review') {
      pull_request_object = utils.formatReviewRequest(pull_request, review_comment);
    }
    else {
      pull_request_object = utils.formatCommentRequest(pull_request, review_comment);
    }
      ...
      list_of_users_to_notify.map(function(assignee_github_user) {
        if (author !== assignee_github_user) {
          ...
          if (assignee) {
            ...
            robot.logger.error('No mapped user for github user ' + assignee_github_user);
          }
        }
      });
    }
