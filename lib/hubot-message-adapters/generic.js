  ...
  for (i = 0, len = users.length; i < len; i++) {
    ...
    if (user) {
      if (_(this.disabledUsers).contains(user.id)) {
        this.robot.logger.debug('Github Notification surpressed for ' + user.name);
      }
      else {
        if ((message.author != null) && user.name === message.author) {
          this.robot.logger.info('Github Notification surpressed for '
